...
import React, { useState } from "react";
// 1. Changed import from 'Link' to 'NavLink' and corrected the library to 'react-router-dom'
...
const Navbar = () => {
  ...
  // Helper function for base classes to avoid repetition
  ...
  return (
    <nav className="w-full z-50 bg-transparent">
      <div className="max-w-7xl mx-auto px-6 py-6 flex items-center justify-between">
        {/* Logo / Brand Name - now uses NavLink */}
        <motion.div
          initial={{ opacity: 0, y: -20 }}
          animate={{ opacity: 1, y: 0 }}
          transition={{ duration: 0.5 }}
        >
          {/* 2. Replaced Link with NavLink */}
          <NavLink
            to="/"
            className="text-heading text-3xl font-extrabold bg-gradient-to-r from-primary via-secondary to-highlight bg-clip-text text-transparent"
          >
            {/* It's good practice to have content inside your link/logo */}
          </NavLink>
        </motion.div>

        {/* Desktop Menu - now uses NavLink with active styles */}
        <div className="hidden md:flex items-center gap-10 ">
          {navItems.map((item, index) => (
            <motion.div
              key={item.name}
              ...
              transition={{ delay: 0.1 * index }}
            >
              {/* 3. Replaced Link with NavLink and added conditional className */}
              <NavLink
                to={item.to}
                className={({ isActive }) =>
                  `text-2xl font-lato ${navLinkBaseClasses} ${
      ...
      <AnimatePresence>
        {isOpen && (
          ...
          >
            <div className="flex flex-col items-center py-6 space-y-5">
              {navItems.map((item) => (
                // Replaced Link with NavLink and added conditional className here as well
                <NavLink
                  key={item.name}
                  to={item.to}
                  className={({ isActive }) =>
                    `text-xl font-semibold ${navLinkBaseClasses} ${
                      isActive ? "text-highlight" : "text-text"
                    }`
                  }
                  onClick={() => setIsOpen(false)}
